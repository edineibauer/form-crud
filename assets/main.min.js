...
if(typeof formSubmit!=='function'){var saveTime;var isSavingNew=!1;function setError($form,erro,novo,t){isSavingNew=!1;t=t||"dados.";$.each(erro,function(c,mensagem){if(typeof(mensagem)==="object"){setError($form,mensagem,novo,t+c+'.')}else{var color=novo?"red":"goldenrod";var $input=$form.find("[data-model='"+(t+c)+"']");if($input.attr("data-format")==="radio"){$input.siblings(".md-radio--fake").css("border-color",color);$input.parent().siblings(".radio-title").addClass(color+"-span")}else if($input.attr("data-format")==="list"){$input.parent().parent().prev().addClass(color+"-span");$input.siblings(".listButton").parent().siblings(".rest").find("input[type=text]").siblings('.error-message').remove();$input.parent().siblings(".rest").append('<span class="'+color+'-span error-message">'+mensagem+'</span>')}else{$input.siblings('label').addClass(color+"-span");$input.siblings('.error-message').remove();$input.addClass(color+"-subErro").parent().append('<span class="'+color+'-span error-message">'+mensagem+'</span>')}}})}
    function cleanError($form){$form.find(".error-span, .red-span, .goldenrod-span, .red-subErro, .goldenrod-subErro").removeClass("red-subErro error-span red-span goldenrod-span");$form.find(".error-message").remove();$form.find(".md-radio--fake").css("border-color","initial")}
    ...
    function formSubmit($form,$idReturn){clearTimeout(saveTime);statusPanel('change',$form);window.onbeforeunload=function(){clearTimeout(saveTime);formSave($form);window.onbeforeunload=null;return!0};if(typeof($idReturn)!=="undefined"){post('form-crud',"save/form",{entity:$form.attr("data-entity"),dados:formGetData($form)},function(data){var title=$form.find("[data-model='dados."+$form.find("input[type=hidden][rel='title']").val()+"']").val();if(["list_mult","extend_mult","selecao_mult","list","selecao"].indexOf($idReturn.attr("data-format"))>-1&&data>0&&parseInt($idReturn.val())!==parseInt(data))
        ...
        $idReturn.val(data).trigger("change");else if($idReturn.val()===""&&$form.find("input[type=hidden][data-model='dados.id']").val()!=="")
        $idReturn.val($form.find("input[type=hidden][data-model='dados.id']").val()).trigger("change");if($idReturn.val()!==""&&typeof($form.find("input[type=hidden][rel='title']").val())==="string")
...
if(typeof formAutoSubmit!=='function'){function formAutoSubmit($element){$element.off("keyup change",".jqte_editor").on("keyup change",".jqte_editor",function(e){if(e.which!==undefined&&[13,37,38,39,40,116].indexOf(e.which)<0)
    ...
    $list.find("li.active").removeClass("active").next().addClass("active")}else if(e.which===13){selectList($list)}}else if([37,39].indexOf(e.which)<0){if($this.val()!==""){readList($this,$this.attr("data-entity"),$this.attr("data-parent"),$this.val(),$this.attr("id"))}else{$this.siblings(".list-complete").html("");$("#list-"+$this.attr("id")).removeClass("color-white").addClass("color-teal").find("i").html("add")}}}).off("dblclick",".form-list").on("dblclick",".form-list",function(){if(!$(this).prop("disabled"))
...
Dropzone.autoDiscover=!1;function loadForm(element){var $element=$(element);$element.off("click","#saveFormButton").on("click","#saveFormButton",function(){formSave($(this).closest(".form-crud"),!0)});loadMask($element);formAutoSubmit($element.closest(".form-control"))}loadForm('.form-crud')